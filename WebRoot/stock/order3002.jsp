...
<html>
  <head>
	...
	<script type="text/javascript">
		function aaa(){
					var outNo1 = $(".outOrderNo2").val();
					//alert("outNo1======"+outNo1);
					if(outNo1 == "自动生成"){
					//alert("ftt");
						document.getElementById('div').innerHTML="请先创建或选中出库单才能新建对应的出库单明细";
						$("#div").css('width' , '450px');
						$("#div").show();
					}
					else{
						...
						var outId = $("#outId").val();
						//alert("outId======"+outId);
						var outNo = $(".outOrderNo2").val();
						if(outId == 0){
							$("#idmig0101").attr("action" ,  "<%=basePath%>outOrderDetail/findAllCloth.action?model=add&id="+outId+"&outNo="+outNo);
							$("#idmig0101").submit(); 
						}else{
							$("#idmig0102").attr("action" ,  "<%=basePath%>outOrderDetail/findAllCloth.action?model=add&id="+outId+"&outNo="+outNo);
							$("#idmig0102").submit(); 
						}
						...
						$.ajax({
			            	...
			            	success:function(data){
								...
								for (var i=0;i<d.length;i++){ 
									alert("d[i].outNo=="+d[i].outNo);
									alert("d[i].id=="+d[i].id);
									var outOrderNo=  d[i].outNo;
									var outId = d[i].id;
									if(outOrderNo != null){
										$('#outOrderNo').val(outOrderNo);
										$('#outId').val(outId);
										$("#div").text("添加成功！");
									}else{
										$("#div").text("添加失败！");
  ...
  <body BACKGROUND="<%=basePath%>/image/bg.gif">
				...
				<table border=0 cellspacing=1 cellpadding=2 width="100%" bgcolor="gray">
					<tr>
						...
						<td class="textbar01" width="35%">
							...
							<input type="text" class="outOrderNo2" id="outOrderNo" name="outOrder.outNo" value="自动生成" readonly="readonly"/>
					    </td>
			...
			<s:if test="#request.model == 'update'">
				<table border=0 cellspacing=1 cellpadding=2 width="100%" bgcolor="gray">
					<tr>
						...
						<td class="textbar01" width="35%">
							<input type="hidden" id="outId" value="${outOrder.id }">
							<input type="text" class="outOrderNo2" id ="outNo" name="outOrder.outNo" value="${outOrder.outNo } " readonly="readonly"/>
					    </td>
					    <td class="textbar81" width="15%">单据日期</td>
						<td class="textbar01" width="35%">
		...
		<form name="form2" ID="idmig0102"  method="post" action="" onsubmit="" >
			        ...
			        <td class="headerbar63"  colspan="1">
			        	<p align="right">
			            	...
			            	<input type=button value=" 新增明细 " onClick="jsgoto('<%=basePath%>/outOrderDetail/findAllCloth.action?outNo=${outOrder.outNo }&id=${outOrder.id }&model=add');">
			            	 -->
			            	 <input type=button value=" 新增明细 " onclick="aaa();">
			            </p>
			        </td>                    
			    </tr>
			</table>
